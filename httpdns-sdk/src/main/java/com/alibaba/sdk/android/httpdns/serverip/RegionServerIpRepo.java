...
public class RegionServerIpRepo {
	...
	private final HashMap<String, RegionServerIpData> mCache = new HashMap<>();

	public String[] getServerIps(String region) {
		region = CommonUtil.fixRegion(region);
		RegionServerIpData data = mCache.get(region);
	...
	public void save(String region, String[] serverIps, int[] ports, String[] serverV6Ips,
					 int[] v6Ports) {
		region = CommonUtil.fixRegion(region);
		RegionServerIpData data = new RegionServerIpData(region, serverIps, ports, serverV6Ips, v6Ports);
