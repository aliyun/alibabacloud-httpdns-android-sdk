...
public class InterpretHostResultRepo {
	...
	public InterpretHostResultRepo(HttpDnsConfig config, ProbeService ipProbeService,
								   RecordDBHelper dbHelper, InterpretHostCacheGroup cacheGroup) {
	...
	private void readFromDB(boolean cleanCache) {
		...
		for (HostRecord record : records) {
			...
			InterpretHostCache cache = mCacheGroup.getCache(record.getCacheKey());
			cache.put(record);
		}
		if (cleanCache) {
		...
		if (!mHttpDnsConfig.getRegion().equals(region)) {
			...
			for (final HostRecord record : records) {
				...
				if (!record.isExpired() && type == RequestIpType.v4) {
					mIpProbeService.probeIpv4(record.getHost(), record.getIps(),
						new ProbeCallback() {
							@Override
	...
	private HostRecord save(String region, String host, RequestIpType type, String extra,
		...
		InterpretHostCache cache = mCacheGroup.getCache(cacheKey);
	...
	public void save(String region, String host, RequestIpType type, String extra, String cacheKey,
					 InterpretHostResponse response) {
		final ArrayList<HostRecord> records = new ArrayList<>();
		switch (type) {
			case v4:
