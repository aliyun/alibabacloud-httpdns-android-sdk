...
public class HttpDnsE2E {
	...
	public void changeRegionWillUpdateServerIp() {
		...
		app.changeRegionTo(otherRegion);
		app.changeRegionTo(otherRegion);
		ServerStatusHelper.hasReceiveRegionChange("修改region会触发更新服务IP请求", app, server, otherRegion);
		// 请求域名解析
		app.requestInterpretHost();
		// 确认是另一个服务接受到请求
		ServerStatusHelper.hasReceiveAppInterpretHostRequest("切换region后，新的服务收到域名解析请求", app,
            server3,
			1);
		...
		ServerStatusHelper.hasReceiveRegionChange("修改region会触发更新服务IP请求", app, server3,
			defaultRegion);
		app.requestInterpretHost();
		ServerStatusHelper.hasReceiveAppInterpretHostRequest("切回region后，原来的服务收到域名解析请求", app,
            server,
			1);
	...
	public void interpretHostRequestWillRetryOnceWhenFailed() {
		// 预置第一次失败，第二次成功
		server.getInterpretHostServer().preSetRequestResponse(app.getRequestHost(), 400,
            "whatever",
			1);
		InterpretHostResponse response = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost());
		...
		app.requestInterpretHost();
		// 判断服务器是否收到两次请求
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult(app, server, 400,
			"whatever");
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("正常模式域名解析失败会重试一次", app,
			server1, response);
	...
	public void interpretHostRequestWillShiftServerIpWhenCurrentServerNotAvailable() {
		...
		ServerStatusHelper.hasReceiveAppInterpretHostRequestButTimeout(app, server);
		// 确认服务1 正常返回
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("服务不可用时，会切换服务IP", app,
			server1, response);
	...
	public void interpretHostRequestWillShiftServerIpWhenCurrentServerDegrade() {
		...
		// 确认服务1 正常返回
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("服务降级时会切换服务IP", app,
            server1,
			response);
	...
	public void whenServerIpsFailTwiceEnterSniffMode() {
		...
		app.requestInterpretHost();
		// 服务接受到一次请求，返回失败
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult(app, server2, 400,
			"whatever", 1, true);
		// 没有接收到第二次请求，即没有重试
	...
	public void whenInterpretHostSuccessExitSniffMode() {
		...
		// 设置一次失败
		server2.getInterpretHostServer().preSetRequestResponse(app.getRequestHost(), 400,
			"whatever", 1);
		app.requestInterpretHost(app.getRequestHost());
		// 一次失败，一次正常 两次
	...
	public void setRegionHasTimeInterval() {
		...
		app.changeRegionTo(otherRegion);
		ServerStatusHelper.hasNotReceiveRegionChange("更新服务IP请求没有进行时间间隔限制", app, server,
			otherRegion);
		...
		// 切换到hk
		app.changeRegionTo(otherRegion);
		ServerStatusHelper.hasReceiveRegionChange("更新服务IP请求超过时间间隔才允许请求", app, server, otherRegion);
		// 切回default
		app.changeRegionTo(defaultRegion);
		ServerStatusHelper.hasReceiveRegionChange("更新服务IP请求超过时间间隔才允许请求", app, server3,
			defaultRegion);
	}
	...
	public void updateServerIpsHasTimeInterval() throws InterruptedException {
		...
		app.requestInterpretHost(app.getRequestHost());
		// 确认服务IP请求触发
		ServerStatusHelper.hasReceiveRegionChange("更新服务IP超过时间间隔才能请求,如果在嗅探模式下，也需要缩短嗅探的时间间隔", app,
	...
	public void updateServerFailWhenRetryAnotherServer() {
		...
		ServerStatusHelper.hasReceiveRegionChange("更新服务IP时，失败了但是没有遍历尝试其它服务", app, server1,
			hkRegion);
		ServerStatusHelper.hasNotReceiveRegionChange("更新服务IP时，一旦有一个成功，就不会尝试其它服务了", app, server2,
			hkRegion);
	}
	...
	public void updateServerAllFailWhenRetryInitServer() {
		...
		// 先更新一次服务IP
		app.changeRegionTo(otherRegion);
		ServerStatusHelper.hasReceiveRegionChange("服务收到更新服务IP请求", app, server, otherRegion);

		// 再触发一次服务IP更新，此时全部失败，会切换会初始服务请求
		app.changeRegionTo(defaultRegion);
		ServerStatusHelper.hasReceiveRegionChange("更新服务IP都失败时，没有切换回初始化服务IP尝试", app, server,
			defaultRegion);
	}
	...
	public void differentAcountUseDifferentInstance() {
		...
		// 应用0切换到 hk
		app.changeRegionTo(otherRegion);
		ServerStatusHelper.hasReceiveRegionChange("应用0切换到 其它region", app, server, otherRegion);
		// 应用1正常使用
		...
		// 请求失败, 短时间不能再次嗅探请求
		app1.requestInterpretHost(app1.getRequestHost());
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult(app1, server2, 400,
			"whatever", 1, true);
		app1.requestInterpretHost(app1.getRequestHost());
		...
		InterpretHostResponse response1 =
            InterpretHostServer.randomInterpretHostResponse(sameHost);
		...
		app.waitForAppThread();
		assertThat("应用0获取自己的结果", server3.getInterpretHostServer()
			.hasRequestForArgWithResult(sameHost, response, 1, true));
		app1.requestInterpretHost(sameHost);
		app1.waitForAppThread();
		assertThat("应用1获取自己的结果", server3.getInterpretHostServer()
			.hasRequestForArgWithResult(sameHost, response1, 1, true));
		assertThat("应用0、应用1结果不干扰", app.requestInterpretHost(sameHost),
			Matchers.not(Matchers.arrayContaining(app1.requestInterpretHost(sameHost))));
	}
	...
	public void supportIpv6Interpret() {
		...
		server.getInterpretHostServer().preSetRequestResponse(
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v6),
			response, -1);
		...
		// 验证服务器收到了请求
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("服务应该收到ipv6域名解析请求", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v6),
			server, response, 1, true);
		// 再次请求，获取服务器返回的结果
		ipv6s = app.requestInterpretHostForIpv6();
		ServerStatusHelper.hasNotReceiveAppInterpretHostRequest("有缓存，不会请求服务器", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v6),
			server);
	...
	public void testSDNS() {
		...
		InterpretHostResponse normalResponse = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost());
		server.getInterpretHostServer().preSetRequestResponse(app.getRequestHost(), normalResponse,
			-1);
		InterpretHostResponse sdnsResponse = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost());
		server.getInterpretHostServer().preSetRequestResponse(
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v4,
		...
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("服务器应该接收到sdns请求", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v4,
		...
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("一般解析和sdns解析互不干扰", app,
			server, normalResponse);
		ips = app.requestInterpretHost();
	...
	public void testGlobalParamsSDNS() {
		...
		// 仅global参数
		InterpretHostResponse gsdnsResponse = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost());
		server.getInterpretHostServer().preSetRequestResponse(
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v4,
		...
		all.putAll(extras);
		InterpretHostResponse sdnsResponse1 = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost());
		server.getInterpretHostServer().preSetRequestResponse(
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost(), RequestIpType.v4,
				all), sdnsResponse1, -1);
		...
		UnitTestUtil.assertIpsEqual("global参数+定制参数，sdns解析结果和预期值一致", result1.getIps(),
			sdnsResponse1.getIps());
		...
		UnitTestUtil.assertIpsEqual("清楚global参数后，sdns解析结果和仅定制参数的解析结果一致", result2.getIps(),
			sdnsResponse.getIps());
	}
	...
	public void preInterpretHostForIpv4() {
		...
		ResolveHostResponse response = ResolveHostServer.randomResolveHostResponse(hostList,
			RequestIpType.v4);

		server.getResolveHostServer().preSetRequestResponse(
			ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.v4), response, 1);

		app.preInterpreHost(hostList, RequestIpType.v4);
		app.waitForAppThread();
		MatcherAssert.assertThat("预解析会触发批量请求", server.getResolveHostServer().hasRequestForArg(
			ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.v4), 1, false));
		...
		UnitTestUtil.assertIpsEqual("预解析ipv4之后，可以直接获取解析结果", ips1,
			response.getItem(host1, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("预解析ipv4之后，可以直接获取解析结果", ips2,
			response.getItem(host2, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("预解析ipv4之后，可以直接获取解析结果", ips3,
			response.getItem(host3, RequestIpType.v4).getIps());
	}
	...
	public void preInterpretHostForIpv6() {
		...
		server.getResolveHostServer().preSetRequestResponse(
			ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.v6), response, 1);

		app.preInterpreHost(hostList, RequestIpType.v6);
		app.waitForAppThread();
		MatcherAssert.assertThat("预解析会触发批量请求", server.getResolveHostServer().hasRequestForArg(
			ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.v6), 1, false));
		...
		UnitTestUtil.assertIpsEqual("预解析ipv6之后，可以直接获取解析结果", ips1,
			response.getItem(host1, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("预解析ipv6之后，可以直接获取解析结果", ips2,
			response.getItem(host2, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("预解析ipv6之后，可以直接获取解析结果", ips3,
			response.getItem(host3, RequestIpType.v6).getIps());
	}
	...
	public void preInterpretHost() {
		...
		server.getResolveHostServer().preSetRequestResponse(
			ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.both), response, 1);

		app.preInterpreHost(hostList, RequestIpType.both);
		app.waitForAppThread();
		MatcherAssert.assertThat("预解析会触发批量请求", server.getResolveHostServer().hasRequestForArg(
			ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.both), 1, false));
	...
	public void preInterpretHostMoreThan5() {
		...
		ResolveHostResponse response1 = ResolveHostServer.randomResolveHostResponse(hostList1,
			RequestIpType.both);
		ResolveHostResponse response2 = ResolveHostServer.randomResolveHostResponse(hostList2,
			RequestIpType.both);

		server.getResolveHostServer().preSetRequestResponse(
			ResolveHostServer.ResolveRequestArg.create(hostList1, RequestIpType.both), response1,
			1);
		server.getResolveHostServer().preSetRequestResponse(
			ResolveHostServer.ResolveRequestArg.create(hostList2, RequestIpType.both), response2,
			1);
		...
		app.preInterpreHost(hostList, RequestIpType.both);
		app.waitForAppThread();
		MatcherAssert.assertThat("预解析会触发批量请求,多于5个，会按5个一组拆分", server.getResolveHostServer()
			.hasRequestForArg(
				ResolveHostServer.ResolveRequestArg.create(hostList1, RequestIpType.both), 1,
				false));
		MatcherAssert.assertThat("预解析会触发批量请求,多于5个，会按5个一组拆分", server.getResolveHostServer()
			.hasRequestForArg(
				ResolveHostServer.ResolveRequestArg.create(hostList2, RequestIpType.both), 1,
				false));
		...
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ips1,
			response1.getItem(host1, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ips2,
			response1.getItem(host2, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ips3,
			response1.getItem(host3, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ips4,
			response1.getItem(host4, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ips5,
			response1.getItem(host5, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("超过5个，预解析之后，可以直接获取解析结果", ips6,
			response2.getItem(host6, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("超过5个，预解析之后，可以直接获取解析结果", ips7,
			response2.getItem(host7, RequestIpType.v4).getIps());
		...
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ipv6s1,
			response1.getItem(host1, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ipv6s2,
			response1.getItem(host2, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ipv6s3,
			response1.getItem(host3, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ipv6s4,
			response1.getItem(host4, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("预解析之后，可以直接获取解析结果", ipv6s5,
			response1.getItem(host5, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("超过5个，预解析之后，可以直接获取解析结果", ipv6s6,
			response2.getItem(host6, RequestIpType.v6).getIps());
		UnitTestUtil.assertIpsEqual("超过5个，预解析之后，可以直接获取解析结果", ipv6s7,
			response2.getItem(host7, RequestIpType.v6).getIps());
	}
	...
	public void testIpTtl() throws InterruptedException {
		...
		UnitTestUtil.assertIpsEmpty("第一次请求，没有缓存，应该返回空", ips);
		// 验证服务器收到了请求
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("当没有缓存时，会异步请求服务器", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost()), server, response, 1,
			true);
		// 再次请求，获取服务器返回的结果
		ips = app.requestInterpretHost();
		ServerStatusHelper.hasNotReceiveAppInterpretHostRequest("当有缓存时，不会请求服务器", app, server);
		...
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("ttl过期后，再次请求会触发网络请求", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost()), server, response, 1,
			true);
		// 再次请求，获取再次请求服务器返回的结果
		ips = app.requestInterpretHost();
	...
	public void testEnableExpiredIp() throws InterruptedException {
		...
		InterpretHostResponse response = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost(), 1);
		InterpretHostResponse response1 = InterpretHostServer.randomInterpretHostResponse(
			app.getRequestHost());
		server.getInterpretHostServer().preSetRequestResponse(app.getRequestHost(), response, 1);
		...
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("ttl过期后，再次请求会触发网络请求", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost()), server, response1
            , 1,
			true);
		// 再次请求，获取再次请求服务器返回的结果
		ips = app.requestInterpretHost();
	...
	public void testIpCacheWhenExpired() throws InterruptedException {
		...
		ips = app.requestInterpretHost();
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("本地缓存过期时，会触发网络请求", app,
			server, response1);
		...
		ips = app.requestInterpretHost();
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("ttl过期，会触发网络请求", app,
            server,
			response1);
	...
	public void testHostFilterForResolve() {
		...
		MatcherAssert.assertThat("因为域名过滤了,不会请求到服务器", !server.getResolveHostServer()
			.hasRequestForArg(ResolveHostServer.ResolveRequestArg.create(list, RequestIpType.v4)
                , 1,
				false));
		list.remove(app.getRequestHost());
		MatcherAssert.assertThat("没过滤的域名会请求到服务器", server.getResolveHostServer()
			.hasRequestForArg(ResolveHostServer.ResolveRequestArg.create(list, RequestIpType.v4)
                , 1,
				false));
	}
	...
	public void testAuthSign() {
		...
		params.add("t");
		params.add("host");
		MatcherAssert.assertThat("请求服务器时，发出的是带签名的请求", server.getInterpretHostServer()
			.hasRequestForArgWithParams(
				InterpretHostServer.InterpretHostArg.create(host, RequestIpType.v4), params, 1,
				false));
	}
	...
	public void testAuthSignValid() {
		...
		params.add("t");
		params.add("host");
		MatcherAssert.assertThat("超过有效期，服务不处理", !server.getInterpretHostServer()
			.hasRequestForArgWithParams(
				InterpretHostServer.InterpretHostArg.create(host, RequestIpType.v4), params, 1,
				false));
	}
	...
	public void testResolveAfterNetworkChanged() {
		...
		app.waitForAppThread();
		UnitTestUtil.assertIpsEqual("网络变化之后，会重新解析已解析的域名", app.requestInterpretHost(host1),
			server.getResolveHostServer().getResponseForHost(host1, RequestIpType.v4)
				.getItem(host1, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("网络变化之后，会重新解析已解析的域名", app.requestInterpretHost(host2),
			server.getResolveHostServer().getResponseForHost(host2, RequestIpType.v4)
				.getItem(host2, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("网络变化之后，会重新解析已解析的域名", app.requestInterpretHost(host3),
			server.getResolveHostServer().getResponseForHost(host3, RequestIpType.v4)
				.getItem(host3, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("网络变化之后，会重新解析已解析的域名", app.requestInterpretHost(host4),
			server.getResolveHostServer().getResponseForHost(host4, RequestIpType.v4)
				.getItem(host4, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("网络变化之后，会重新解析已解析的域名", app.requestInterpretHost(host5),
			server.getResolveHostServer().getResponseForHost(host5, RequestIpType.v4)
				.getItem(host5, RequestIpType.v4).getIps());
		UnitTestUtil.assertIpsEqual("网络变化之后，会重新解析已解析的域名", app.requestInterpretHost(host6),
			server.getResolveHostServer().getResponseForHost(host6, RequestIpType.v4)
				.getItem(host6, RequestIpType.v4).getIps());
	...
	public void testNotResolveWhenNetworkDisconnect() {
		...
		UnitTestUtil.assertIpsEqual("网络断开再连上相同网络，解析的缓存不变", app.requestInterpretHost(host1),
			server.getInterpretHostServer().getResponse(host1, 1, false).get(0).getIps());
		UnitTestUtil.assertIpsEqual("网络断开再连上相同网络，解析的缓存不变", app.requestInterpretHost(host2),
			server.getInterpretHostServer().getResponse(host2, 1, false).get(0).getIps());
		UnitTestUtil.assertIpsEqual("网络断开再连上相同网络，解析的缓存不变", app.requestInterpretHost(host3),
			server.getInterpretHostServer().getResponse(host3, 1, false).get(0).getIps());
		UnitTestUtil.assertIpsEqual("网络断开再连上相同网络，解析的缓存不变", app.requestInterpretHost(host4),
			server.getInterpretHostServer().getResponse(host4, 1, false).get(0).getIps());
		UnitTestUtil.assertIpsEqual("网络断开再连上相同网络，解析的缓存不变", app.requestInterpretHost(host5),
			server.getInterpretHostServer().getResponse(host5, 1, false).get(0).getIps());
		UnitTestUtil.assertIpsEqual("网络断开再连上相同网络，解析的缓存不变", app.requestInterpretHost(host6),
			server.getInterpretHostServer().getResponse(host6, 1, false).get(0).getIps());
	}
	...
	public void testMultiThreadForSameHost() {
		...
		app.waitForAppThread();
		MatcherAssert.assertThat("预解析相同的域名也只会触发一次请求", server.getResolveHostServer()
			.hasRequestForArg(
				ResolveHostServer.ResolveRequestArg.create(hostList, RequestIpType.both), 1,
				false));
	}
			...
			for (int i = 0; i < 1000; i++) {
				...
				switch (i % 7) {
					...
					case 6:
						...
						app.requestSDNSInterpretHost(extras,
	...
	public void multiThreadTest() {
		...
		for (int i = 0; i < hostCount; i++) {
			if (hosts.get(i).startsWith(timeoutPrefix)) {
				...
				// random response
				server.getInterpretHostServer().preSetRequestResponse(hosts.get(i),
					InterpretHostServer.randomInterpretHostResponse(hosts.get(i), 5), -1);
			}
		}
	...
	public void stopInterpretHostWhenServerIpDoNotBelongCurrentRegion() {
		...
		ServerStatusHelper.hasNotReceiveAppInterpretHostRequest(
			"服务IP和region不匹配, 应该停止解析，直接返回，不应该请求当前服务IP进行解析", app, server);
		ServerStatusHelper.hasNotReceiveAppInterpretHostRequest("region更新还未完成，应该不会请求到新的服务IP", app,
			server3);
	}
	...
	public void enableCacheWhenInit() {
		...
		String[] ips2 = app.requestInterpretHost();
		ServerStatusHelper.hasReceiveAppInterpretHostRequest("有缓存，但是由于无法判断之前和之后的网络是否一致，还是需要再请求一次",
			app, server, 2);
	...
	public void disableExpireIpWhenInit() throws InterruptedException {
		...
		ServerStatusHelper.hasReceiveAppInterpretHostRequestWithResult("ttl过期后，请求会触发网络请求", app,
			InterpretHostServer.InterpretHostArg.create(app.getRequestHost()), server, response1
            , 1,
			true);
		// 再次请求，获取再次请求服务器返回的结果
