...
public class V2_3_0_config {
    ...
    public void testUpdateServerFallbackToDefaultUpdateServer() {
        ...
        app.configInitServer(REGION_DEFAULT, new HttpDnsServer[]{server}, new HttpDnsServer[]{server1, server2});
        // 初始化httpdns 初始化调度请求
        app.start(false);

        app.waitForAppThread();
        assertThat("调度请求失败会回退到默认调度IP请求, region " + REGION_DEFAULT, server2.getServerIpsServer().hasRequestForArg(REGION_DEFAULT, 1, true));
    }
